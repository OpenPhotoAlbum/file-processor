...
export class HeritageProcessor extends BaseProcessor {
  ...
  private logger = new Logger('HeritageProcessor');
    ...
    try {
      ...
      const locationResult = {
        ...
        enrichmentStatus: {
          ...
          landmarks: 'disabled' as const // Override landmarks to disabled for performance
        }
      };
      ...
      this.systemErrors.fileOperationFailed({ filePath: file.path }, err as Error);
      throw err;
    }
  }

  private parseSeriesInfo(documentName?: string): { seriesName?: string; totalPages?: number } | null {
  ...
  private async loadTranscription(filePath: string): Promise<string | null> {
    try {
  ...
  private detectCreatorType(creator?: string): string {
