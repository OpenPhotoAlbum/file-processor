...
export class ImageValidator {
     ...
     */
    async readFileSignature(filePath) {
     ...
     */
    detectSignature(buffer) {
        const hex = buffer.toString('hex').toLowerCase();
        // JPEG
        if (hex.startsWith(FILE_SIGNATURES.JPEG))
            return 'jpeg';
        // PNG
        if (hex.startsWith(FILE_SIGNATURES.PNG))
            return 'png';
        // GIF (both 87a and 89a versions)
        if (hex.startsWith(FILE_SIGNATURES.GIF_87A) || hex.startsWith(FILE_SIGNATURES.GIF_89A))
            return 'gif';
        // TIFF (little-endian and big-endian)
        if (hex.startsWith(FILE_SIGNATURES.TIFF_LITTLE_ENDIAN) || hex.startsWith(FILE_SIGNATURES.TIFF_BIG_ENDIAN))
            return 'tiff';
        ...
        // BMP
        if (hex.startsWith(FILE_SIGNATURES.BMP))
            return 'bmp';
     ...
     */
    async readFileCompletely(filePath) {
