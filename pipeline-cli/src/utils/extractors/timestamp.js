...
export class TimestampExtractor {
    logger = new Logger('Timestamp Extractor');
    timestampErrors = createTimestampErrorFactory(this.logger);
    /**
     * Main timestamp extraction method
     */
    async extractTimestamps(sources) {
        ...
        try {
            if (sidecar.source === SidecarSource.GOOGLE_TAKEOUT && sidecar.format === SidecarFormat.JSON) {
                ...
                if (data.photoTakenTime && typeof data.photoTakenTime === 'object' && data.photoTakenTime !== null) {
                    const photoTakenTime = data.photoTakenTime;
                    if (photoTakenTime.timestamp) {
                        timestamps.push({
                            timestamp: this.normalizeUnixTimestamp(photoTakenTime.timestamp),
                            source: 'sidecar',
                            confidence: 'high',
                ...
                if (data.creationTime && typeof data.creationTime === 'object' && data.creationTime !== null) {
                    ...
                    if (creationTime.timestamp) {
                        timestamps.push({
                            timestamp: this.normalizeUnixTimestamp(creationTime.timestamp),
                            source: 'sidecar',
                            confidence: 'medium',
     ...
     */
    extractGenericSidecarTimestamps(sidecar) {
        const timestamps = [];
        const data = sidecar.data;
     ...
     */
    extractFromFilesystem(fileStats) {
        const timestamps = [];
     ...
     */
    analyzeTimestamps(allTimestamps) {
     ...
     */
    sortTimestampsByPriority(timestamps) {
        ...
        return [...timestamps].sort((a, b) => {
            ...
            if (a.sourceDetails.includes('DateTimeOriginal'))
                return -1;
            if (b.sourceDetails.includes('DateTimeOriginal'))
                return 1;
            return 0;
        });
    }
     ...
     */
    findBestCaptureTimestamp(sortedTimestamps) {
     ...
     */
    findBestCreationTimestamp(sortedTimestamps) {
     ...
     */
    findBestModificationTimestamp(sortedTimestamps) {
     ...
     */
    detectTimestampConflicts(timestamps) {
        const conflicts = [];
        ...
        if (highConfidence.length >= 2) {
            for (let i = 0; i < highConfidence.length - 1; i++) {
                for (let j = i + 1; j < highConfidence.length; j++) {
                    const diff = this.getTimestampDifferenceHours(highConfidence[i].timestamp, highConfidence[j].timestamp);
                    // Flag conflicts > 24 hours between high-confidence sources
                    if (Math.abs(diff) > 24) {
                        conflicts.push(`${highConfidence[i].sourceDetails} vs ${highConfidence[j].sourceDetails}: ${Math.round(diff)}h difference`);
                    }
                }
            }
        }
     ...
     */
    getTimestampDifferenceHours(timestamp1, timestamp2) {
