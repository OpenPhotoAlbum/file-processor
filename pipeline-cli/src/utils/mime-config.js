 ...
 */
export function getEnabledMimeTypes() {
    ...
    for (const requestedType of requestedTypes) {
        const isSupported = allSupported.some(supported => supported.toLowerCase() === requestedType);
        if (isSupported) {
            // Find the correct case version from the enum
            const correctCase = allSupported.find(supported => supported.toLowerCase() === requestedType);
            if (correctCase) {
 ...
 */
export function isMimeTypeEnabled(mimeType) {
...
//# sourceMappingURL=mime-config.js.map