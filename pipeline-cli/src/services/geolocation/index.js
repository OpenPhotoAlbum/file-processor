...
export class GeolocationService {
    config;
    logger = new Logger('Geolocation Service');
     ...
     */
    async findByBoundaryContainment(lat, lng) {
        if (!this.dbPool) {
    ...
    `;
        try {
            ...
            this.logger.warn('Boundary containment query failed', { lat, lng, error: error.message });
            return null;
        }
    }
    /**
     ...
     */
    async findByBoundaryProximity(lat, lng) {
        if (!this.dbPool)
            return null;
    ...
    `;
        try {
            ...
            this.logger.warn('Boundary proximity query failed', { lat, lng, error: error.message });
            return null;
        }
    }
    /**
     ...
     */
    async findByCityProximity(lat, lng) {
        if (!this.dbPool)
            return null;
        const maxDistanceMeters = this.config.maxRadiusKm * 1000;
        const query = `
    ...
    `;
        try {
            ...
            this.logger.warn('City proximity query failed', { lat, lng, error: error.message });
            return null;
        }
    }
    /**
     ...
     */
    calculateProximityConfidence(distanceMeters) {
