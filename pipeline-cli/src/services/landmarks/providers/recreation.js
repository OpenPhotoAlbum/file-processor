 ...
 */
import { LandmarkCategory, LandmarkSource, LandmarkRelationship } from '../types.js';
import { Logger } from '../../../utils/logging/index.js';
export class RecreationProvider {
    name = 'Recreation.gov';
    priority = 3; // After National Parks (1) and GNIS (2)
    categories = [
    ...
    async searchFacilities(lat, lng, radius, limit) {
    ...
    async searchRecreationAreas(lat, lng, radius, limit) {
    ...
    mapFacilityToLandmark(facility, searchLat, searchLng) {
        // Calculate distance using haversine formula
        const distance = this.calculateDistance(searchLat, searchLng, facility.FacilityLatitude, facility.FacilityLongitude);
        ...
        if (facility.FacilityPhone || facility.FacilityAdaAccess || facility.StayLimit) {
            ...
            if (facility.FacilityPhone)
                metadata.push(`Phone: ${facility.FacilityPhone}`);
            if (facility.FacilityAdaAccess)
                metadata.push(`ADA Access: ${facility.FacilityAdaAccess}`);
            if (facility.StayLimit)
                metadata.push(`Stay Limit: ${facility.StayLimit}`);
    ...
    mapCampsiteToLandmark(campsite, searchLat, searchLng) {
        ...
        if (campsite.PERMITTEDEQUIPMENT?.length) {
            const equipment = campsite.PERMITTEDEQUIPMENT.map(eq => `${eq.EquipmentName}${eq.MaxLength ? ` (${eq.MaxLength}ft max)` : ''}`).join(', ');
    ...
    mapRecreationAreaToLandmark(area, searchLat, searchLng) {
        // Calculate distance using haversine formula
        const distance = this.calculateDistance(searchLat, searchLng, area.RecAreaLatitude, area.RecAreaLongitude);
        ...
        if (area.RecAreaPhone || area.RecAreaEmail) {
            const contact = [];
            if (area.RecAreaPhone)
                contact.push(`Phone: ${area.RecAreaPhone}`);
            if (area.RecAreaEmail)
                contact.push(`Email: ${area.RecAreaEmail}`);
    ...
    mapRecreationAreaToCategory(area) {
    ...
    calculateRecreationAreaConfidence(distance, area) {
    ...
    mapFacilityTypeToCategory(facilityType, facilityName) {
    ...
    calculateConfidence(distance, category, facilityType) {
    ...
    determineRelationship(distance) {
        if (distance <= 500)
