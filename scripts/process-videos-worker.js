...
/**
 * Google Takeout Video Processing Worker
 * 
 * Processes video files from the timestamp extraction results using a job queue
 * with file locking for parallel-safe execution.
 * 
 * Usage:
 *   node scripts/process-videos-worker.js [--test-single]
 *   
 *   --test-single: Process only one file and exit (for testing)
 * 
 * Parallel execution:
 *   parallel -j 20 node scripts/process-videos-worker.js ::: {1..20}
 */
...
// Configuration
const WORK_DIR = '/tmp/google_takeout_processing_videos';
...
/**
 * Create collection symlinks for a video
 */
...
/**
 * Process a single file pair (video file + timestamp)
 */
...
async function main() {
  ...
  while (true) {
    ...
    try {
      ...
      if (success) {
        log(`Successfully processed video file ${filesProcessed}`);
      }
  ...
  log(`Video worker finished. Total files processed: ${filesProcessed}`);
}
